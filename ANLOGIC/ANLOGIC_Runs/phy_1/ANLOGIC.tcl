...
set rc [catch {
  ...
  import_db {../syn_1/ANLOGIC_gate.db}
  read_sdc ../../ANLOGIC.sdc
  place
  export_db {ANLOGIC_place.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  route
  report_area -io_info -file ANLOGIC_phy.area
  export_db {ANLOGIC_pr.db}
  start_timer
  report_timing -mode FINAL -net_info  -rpt_autogen true -file ANLOGIC_phy.timing
} RESULT]
if {$rc} {
...
set rc [catch {
  export_bid ANLOGIC_inst.bid
  bitgen -bit "ANLOGIC.bit"
} RESULT]
if {$rc} {
  step_error bitgen
  return -code error $RESULT
